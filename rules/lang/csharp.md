...
- テストの命名は testing.md「テストの命名」を次のとおり割り当てる
  - テスト対象の要素 = クラスの `[Trait("対象", "...")]` に日本語で書く (例: `[Trait("対象", "送料計算")]`)。クラス名は英語でよく、仕様ドキュメントには出さない。同じ前提のケースが多いときは Given / When をネストクラスで階層に切る
  - 各ケースの名前 = `[Fact(DisplayName = "...")]` / `[Theory(DisplayName = "...")]` に日本語の自由文で書く
  - `[Theory]` の各ケースは引数値がテスト名に展開される。値だけで Given + Then が読み取れない場合は、ケース名の文字列を先頭引数に加える
