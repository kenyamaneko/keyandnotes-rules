...
文章規範の各節 (「整形」〜「見出しの付け方」) の原典: [日本語技術文書の文章規範](https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d)。原文をそのまま借用し、書籍原稿に固有の規範 (一文一行、脚注、語り・演出の管理など) を落としている。「ADR の構成」と、「見出しの付け方」の章節番号のルールは本リポジトリ独自の規約。
...
- 章や節に番号を打たない（「2. 背景」「2.1 制約」のような見出しにしない）。番号で参照された箇所は、章節の追加・削除・並べ替えのたびに全参照を追従させる必要があり、保守が難しくなるため。
